 ...
 * @author Admin
 */
public class UpdateVeterinaryAgency extends javax.swing.JDialog {
    private VeterinaryAgencyForm owner;  // Form cha để tải lại dữ liệu sau khi cập nhật
    private VeterinaryAgency currentAgency;  // Đại lý hiện tại để chỉnh sửa

    public UpdateVeterinaryAgency(javax.swing.JInternalFrame parent, javax.swing.JFrame owner, boolean modal) {
        super(owner, modal);
        this.owner = (VeterinaryAgencyForm) parent;
        initComponents();
        setLocationRelativeTo(null);

        // Lấy thông tin đại lý được chọn từ bảng
        currentAgency = this.owner.getAgencySelected();

        // Điền thông tin đại lý vào các trường nhập liệu
        if (currentAgency != null) {
            txtName.setText(currentAgency.getName());
            txtAddress.setText(currentAgency.getAddress());
            txtEmail.setText(currentAgency.getEmail());
            txtLicenseNumber.setText(currentAgency.getLicenseNumber());
            if ("Active".equalsIgnoreCase(currentAgency.getStatus())) {
                jRadioButton1.setSelected(true);
            } else {
                jRadioButton1.setSelected(true);
            }
        } else {
            JOptionPane.showMessageDialog(this, "Không có đại lý nào được chọn!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            dispose();
        }
    }

    private UpdateVeterinaryAgency(JFrame jFrame, boolean b) {
        throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
    }
// Lấy dữ liệu từ các trường nhập
    private VeterinaryAgency getUpdatedAgency() {
        ...
        currentAgency.setName(name);
        currentAgency.setAddress(address);
        currentAgency.setEmail(email);
        currentAgency.setLicenseNumber(licenseNumber);
        currentAgency.setStatus(status);
    ...
    private void btnAddProductActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddProductActionPerformed
        VeterinaryAgency updatedAgency = getUpdatedAgency();
        if (updatedAgency != null) {
            int result = VeterinaryAgencyDAO.getInstance().update(updatedAgency);
            if (result > 0) {
                JOptionPane.showMessageDialog(this, "Cập nhật đại lý thành công!", "Thành công", JOptionPane.INFORMATION_MESSAGE);
                owner.loadDataToTable((ArrayList<VeterinaryAgency>) VeterinaryAgencyDAO.getInstance().selectAll());
                dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Cập nhật đại lý thất bại!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        }
...
//    }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                UpdateVeterinaryAgency dialog = new UpdateVeterinaryAgency(new javax.swing.JFrame(), true);
