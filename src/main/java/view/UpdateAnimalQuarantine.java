 ...
 * @author Admin
 */
public class UpdateAnimalQuarantine extends javax.swing.JDialog {
    private AnimalQuarantineForm owner; // Form cha để tải lại dữ liệu sau khi cập nhật

    public UpdateAnimalQuarantine(javax.swing.JInternalFrame parent, javax.swing.JFrame owner, boolean modal) {
        super(owner, modal);
        initComponents();
        setLocationRelativeTo(null);

        // Truyền form cha từ `AnimalQuarantineForm`
        this.owner = (AnimalQuarantineForm) parent;

        // Lấy thông tin `AnimalQuarantine` đã được chọn từ bảng
        AnimalQuarantine quarantine = this.owner.getQuarantineSelected();

        // Điền thông tin vào các trường
        txtId.setText(String.valueOf(quarantine.getQuarantineId())); // Mã khu tạm giữ
        txtName.setText(quarantine.getLocationName()); // Tên địa điểm
        txtAddress.setText(quarantine.getAddress()); // Địa chỉ
        txtCapacity.setText(String.valueOf(quarantine.getCapacity())); // Sức chứa

        // Thiết lập loại
        cbxType.removeAllItems();
        cbxType.addItem("Chọn loại");
        cbxType.addItem("Gia súc");
        cbxType.addItem("Gia cầm");
        cbxType.addItem("Khác");
        cbxType.setSelectedItem(quarantine.getType()); // Đặt loại đã chọn

        // Thiết lập trạng thái
        if (quarantine.getStatus().equalsIgnoreCase("In Use")) {
            jRadioButton1.setSelected(true);
        } else if (quarantine.getStatus().equalsIgnoreCase("Available")) {
            jRadioButton1.setSelected(true);
        }
    }

    private UpdateAnimalQuarantine(JFrame jFrame, boolean b) {
        throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
    }
    // Phương thức lấy dữ liệu người dùng nhập
    private AnimalQuarantine getQuarantineFromInput() {
        ...
        if (name.isEmpty() || address.isEmpty() || type.equals("Chọn loại") || txtCapacity.getText().isEmpty()) {
        ...
        return new AnimalQuarantine(id, name, address, type, capacity, status);
    }
    ...
    private void btnAddProductActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddProductActionPerformed
        AnimalQuarantine updatedQuarantine = getQuarantineFromInput();
        if (updatedQuarantine != null) {
            int result = AnimalQuarantineDAO.getInstance().update(updatedQuarantine);
            if (result > 0) {
                JOptionPane.showMessageDialog(this, "Cập nhật thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                owner.loadDataToTable((ArrayList<AnimalQuarantine>) AnimalQuarantineDAO.getInstance().selectAll());
                dispose(); // Đóng dialog sau khi cập nhật
            } else {
...
//    }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                UpdateAnimalQuarantine dialog = new UpdateAnimalQuarantine(new javax.swing.JFrame(), true);
