 ...
 * @author Admin
 */
public class QuarantineZoneForm extends javax.swing.JInternalFrame {
   // Model table cho LivestockHealth
    private DefaultTableModel tblModel;
    private ArrayList<LivestockHealth> healthList;

    public QuarantineZoneForm() {
        initComponents();
        UIManager.put("Table.showVerticalLines", true);
        BasicInternalFrameUI ui = (BasicInternalFrameUI) this.getUI();
        ui.setNorthPane(null);

        tblHealth.setDefaultEditor(Object.class, null); // Không cho chỉnh sửa trực tiếp trong bảng
        initTable();
    ...
    public final void initTable() {
        ...
        String[] headerTbl = new String[]{
            "ID", "Farm ID", "Tên bệnh", "Ngày báo cáo", "Ngày kết thúc", "Trạng thái", "Ghi chú"
        };
        tblModel.setColumnIdentifiers(headerTbl);
        tblHealth.setModel(tblModel);
    }
    // Phương thức lấy chứng chỉ được chọn từ bảng
      // Phương thức load dữ liệu `LivestockHealth` vào bảng
    public void loadDataToTable(ArrayList<LivestockHealth> healthList) {
        try {
            tblModel.setRowCount(0); // Xóa các dòng cũ trong bảng
            for (LivestockHealth health : healthList) {
                tblModel.addRow(new Object[]{
                    health.getHealthId(),
                    health.getFarmId(),
                    health.getDiseaseName(),
                    health.getReportedDate(),
                    health.getResolvedDate() == null ? "Chưa kết thúc" : health.getResolvedDate(),
                    health.getStatus(),
                    health.getComments()
                });
            }
    ...
    // Phương thức lấy LivestockHealth được chọn từ bảng
    public LivestockHealth getHealthSelected() {
        int i_row = tblHealth.getSelectedRow();
        if (i_row >= 0) {
            int healthId = (int) tblHealth.getValueAt(i_row, 0); // Lấy `health_id` từ cột đầu tiên
            return LivestockHealthDAO.getInstance().selectById(healthId);
        }
    ...
    private void initComponents() {
        ...
        tblHealth.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        tblHealth.setGridColor(new java.awt.Color(204, 204, 204));
        tblHealth.setShowGrid(true);
    ...
    private void btnDeleteAccountActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteAccountActionPerformed
        if (tblHealth.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn bệnh dịch cần xóa!");
        } else {
            int healthId = (int) tblHealth.getValueAt(tblHealth.getSelectedRow(), 0); // Lấy `health_id` từ cột đầu tiên

            int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn xóa bệnh dịch này?", "Xác nhận xóa", JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
                try {
                    int result = LivestockHealthDAO.getInstance().delete(healthId);
                    if (result > 0) {
                        JOptionPane.showMessageDialog(this, "Xóa bệnh dịch thành công!");
                        loadDataToTable((ArrayList<LivestockHealth>) LivestockHealthDAO.getInstance().selectAll());
                    } else {
                        JOptionPane.showMessageDialog(this, "Không thể xóa bệnh dịch này!");
                    }
                } catch (Exception e) {
                    JOptionPane.showMessageDialog(this, "Lỗi khi xóa bệnh dịch: " + e.getMessage());
                }
            }
        }
    ...
    private void exportExcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exportExcelActionPerformed
        try {
            ...
            if (saveFile != null) {
                ...
                for (int i = 0; i < tblHealth.getColumnCount(); i++) {
                    Cell cell = rowCol.createCell(i);
                    cell.setCellValue(tblHealth.getColumnName(i));
                }

                // Nạp dữ liệu từ bảng vào file Excel
                for (int j = 0; j < tblHealth.getRowCount(); j++) {
                    Row row = sheet.createRow(j + 1);
                    for (int k = 0; k < tblHealth.getColumnCount(); k++) {
                        Cell cell = row.createCell(k);
                        if (tblHealth.getValueAt(j, k) != null) {
                            cell.setCellValue(tblHealth.getValueAt(j, k).toString());
                        }
                    }
                }
