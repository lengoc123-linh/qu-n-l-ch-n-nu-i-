 ...
 * @author Admin
 */
public class UpdateSlaughterhouse extends javax.swing.JDialog {
    private SlaughterhouseForm owner; // Form cha để cập nhật lại dữ liệu sau khi chỉnh sửa

    public UpdateSlaughterhouse(javax.swing.JInternalFrame parent, javax.swing.JFrame owner, boolean modal) {
        super(owner, modal);
        this.owner = (SlaughterhouseForm) parent;
        initComponents();
        setLocationRelativeTo(null);
        initTypeComboBox(); // Thiết lập ComboBox Type

        // Lấy thông tin lò mổ đã chọn từ bảng
        Slaughterhouse selectedSlaughterhouse = this.owner.getSlaughterhouseSelected();
        if (selectedSlaughterhouse != null) {
            loadSlaughterhouseDetails(selectedSlaughterhouse);
        } else {
            JOptionPane.showMessageDialog(this, "Không có lò mổ nào được chọn!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            dispose(); // Đóng form nếu không có dữ liệu được chọn
        }
    }
    ...
    // Load chi tiết của Slaughterhouse đã chọn vào các trường nhập liệu
    private void loadSlaughterhouseDetails(Slaughterhouse slaughterhouse) {
         jLabel8.setText(String.valueOf(slaughterhouse.getId()));
        txtName.setText(slaughterhouse.getName());
        txtAddress.setText(slaughterhouse.getAddress());
        txtPhone.setText(slaughterhouse.getPhone());
        cbxType.setSelectedItem(slaughterhouse.getType());
        txtCapacity.setText(String.valueOf(slaughterhouse.getCapacity()));
        if ("Active".equalsIgnoreCase(slaughterhouse.getStatus())) {
            jRadioButton1.setSelected(true);
        } else {
            jRadioButton2.setSelected(true);
        }
    }
      // Lấy thông tin lò mổ từ form nhập liệu
    private Slaughterhouse getSlaughterhouseFromInput(Slaughterhouse originalSlaughterhouse) {
        ...
        if (name.isEmpty() || address.isEmpty() || phone.isEmpty() || capacityStr.isEmpty() || type.isEmpty()) {
        ...
        return new Slaughterhouse(
            originalSlaughterhouse.getId(), // Giữ nguyên ID của lò mổ
            name, address, phone, type, capacity, status
        );
    }
    ...
    private void initComponents() {
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            ...
                            .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                ...
                                .addComponent(cbxType, 0, 246, Short.MAX_VALUE)))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
        ...
        getContentPane().add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 60, 380, 460));
    ...
    private void btnAddProductActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddProductActionPerformed
        Slaughterhouse originalSlaughterhouse = owner.getSlaughterhouseSelected();
        if (originalSlaughterhouse != null) {
            Slaughterhouse updatedSlaughterhouse = getSlaughterhouseFromInput(originalSlaughterhouse);
            if (updatedSlaughterhouse != null) {
                int result = SlaughterhouseDAO.getInstance().update(updatedSlaughterhouse);
                if (result > 0) {
                    JOptionPane.showMessageDialog(this, "Cập nhật lò mổ thành công!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                    owner.loadDataToTable((ArrayList<Slaughterhouse>) SlaughterhouseDAO.getInstance().selectAll());
                    dispose(); // Đóng form sau khi lưu
                } else {
                    JOptionPane.showMessageDialog(this, "Lỗi khi cập nhật lò mổ!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            }
        }
