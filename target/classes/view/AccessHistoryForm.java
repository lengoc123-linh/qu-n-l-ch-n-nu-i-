 ...
 * @author Admin
 */
public class AccessHistoryForm extends javax.swing.JInternalFrame {

    /**
     * Creates new form AccountForm
     */
    private DefaultTableModel tblModel;
    private ArrayList<UserAccessHistory> accesshistory = UserAccessHistoryDAO.getInstance().selectAll();

    public AccessHistoryForm() {
        initComponents();
    ...
    public final void initTable() {
        ...
        String[] headerTbl = new String[]{"Access ID", "User Name", "Access Time", "Farm ID", "IP address","Device","action"};
        tblModel.setColumnIdentifiers(headerTbl);
        tblAccount.setModel(tblModel);
    }

    public void loadDataToTable(ArrayList<UserAccessHistory> histories) {
    try {
        // Xóa các hàng cũ trong bảng
        tblModel.setRowCount(0);
        
        // Thêm từng lịch sử truy cập vào bảng
            for (UserAccessHistory history : histories) {
    ...
    public UserAccessHistory getAccountSelect() {
        int i_row = tblAccount.getSelectedRow();
        UserAccessHistory acc = UserAccessHistoryDAO.getInstance().selectById(tblAccount.getValueAt(i_row, 1).toString());
        return acc;
    }
    ...
    private void initComponents() {
        ...
        tblAccount.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Access ID ", "User Name", "Accsess Time", "Farm ID", "IP Address", "Device", "Action"
            }
        ));
    ...
    private void btnDeleteAccessActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteAccessActionPerformed
       // TODO add your handling code here:
        if (tblAccount.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn lịch sử truy cập cần xoá !");
        } else {
            UserAccessHistory select = getAccountSelect();
            int checkVl = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn xóa lịch sử truy cập này ?", "Xác nhận xóa", JOptionPane.YES_NO_OPTION);
            if (checkVl == JOptionPane.YES_OPTION) {
                try {                        
                    UserAccessHistoryDAO.getInstance().delete(select);
                    JOptionPane.showMessageDialog(this, "Xoá lịch sử truy cập thành công !");
                    loadDataToTable(UserAccessHistoryDAO.getInstance().selectAll());  // Tải lại dữ liệu vào bảng
                } catch (Exception e) {
                    JOptionPane.showMessageDialog(this, "Xoá lịch sử truy cập thất bại !");
                    e.printStackTrace();
                }
            }
        }
    ...
    private void txtSearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtSearchKeyReleased
            // TODO add your handling code here:
         String luachon = (String) cbxLuachon.getSelectedItem(); // Lấy lựa chọn từ combobox
         String searchContent = txtSearch.getText(); // Lấy nội dung tìm kiếm
         ArrayList<UserAccessHistory> result = new ArrayList<>();

         // Chuyển đổi tùy theo lựa chọn tìm kiếm
         switch (luachon) {
             case "Tất cả":
                 result = SearchUserAccessHistory.getInstance().searchTatCaUserAccessHistory(searchContent); // Tìm kiếm tất cả
                 break;
             case "Tên người dùng":  // Tìm kiếm theo tên người dùng
                 result = SearchUserAccessHistory.getInstance().searchByUserName(searchContent); 
                 break;
             case "Mã cơ sở farm":  // Tìm kiếm theo mã farm
                 try {
                     Integer farmId = Integer.parseInt(searchContent); // Chuyển đổi text thành Integer
                     result = SearchUserAccessHistory.getInstance().searchByFarmId(farmId); 
                 } catch (NumberFormatException e) {
                     JOptionPane.showMessageDialog(this, "Mã cơ sở farm không hợp lệ!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                 }
                 break;
             case "Thời gian truy cập":  // Tìm kiếm theo thời gian truy cập
                 result = SearchUserAccessHistory.getInstance().searchByAccessTime(searchContent); 
                 break;
             case "Hành động":  // Tìm kiếm theo hành động
                 result = SearchUserAccessHistory.getInstance().searchByAction(searchContent); 
                 break;
    }

    // Gọi phương thức loadDataToTable để hiển thị kết quả tìm kiếm
